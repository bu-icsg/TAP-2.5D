...
if (@ARGV == 6) {
	...
	foreach (@AoA_flp){
		...
		if ($w>$tot_x/$smallest_shown && $h>$tot_y/$smallest_shown){
			...
			'" fill="black" text_anchor="start" style="font-size:280" > '. @{$_}[0] .' </text>'."\n";
		}
	}
	}
...
#			$level=int(($max_t-(@{$_}[5]))/($max_t-$min_t)*($num_levels-1));
			...
			if ($i%3==0) {
				...
				$scale_value=($max_t-$min_t)*(1-$i/($num_levels-1))+$min_t-273.15;
				$scale_value=~s/^(\d+)\.(\d)(\d)\d+/$1\.$2$3/;
				$fp.="\t".'<text x="'.$scale_xmin .'" y="'. $txt_ymin.
				'" fill="black" text_anchor="start" style="font-size:1000" > '. $scale_value .' </text>'."\n";
			}
		}
		$clr_ymin+=$h2;
	}
	...
	'" fill="black" text_anchor="start" style="font-size:1000" > '. $min_t .' </text>'."\n";
	}
